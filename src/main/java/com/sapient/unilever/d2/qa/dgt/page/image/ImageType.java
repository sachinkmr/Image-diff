...
public class ImageType extends Featurable {
	...
	if (this.getWebDriver() instanceof InternetExplorerDriver) {
	 ...
	 shot = new
	 AShot().shootingStrategy(ShootingStrategies.viewportNonRetina(AppConstants.SCROLL_DELAY,
	 AppConstants.HEADER_PIXELS,
	 AppConstants.FOOTER_PIXELS)).takeScreenshot(this.getWebDriver());
//	shot = new AShot().shootingStrategy(ShootingStrategies.viewportPasting(AppConstants.SCROLL_DELAY))
