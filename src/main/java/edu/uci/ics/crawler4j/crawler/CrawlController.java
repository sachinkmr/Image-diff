...
public class CrawlController extends Configurable {
	...
	finished = false;
	shuttingDown = false;
	this.addSeed(config.getSiteUrl());
    }
    ...
    /**
     * Start the crawling session and return immediately. This method utilizes
     * default crawler factory that creates new crawler using Java reflection
     *
     * @param _c
     *            the class that implements the logic for crawler threads
     * @param numberOfCrawlers
		...
		public void run() {
			...
			synchronized (waitingLock) {
				...
				if (!someoneIsWorking && shut_on_empty) {
				    ...
				    // alive.
				    logger.info(
					    "It looks like no thread is working, waiting for 5 seconds to make sure...");
				    sleep(5);
     ...
     *            the URL of the seed
     */
    private void addSeed(String pageUrl) {
